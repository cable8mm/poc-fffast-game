...
HelloWorld::HelloWorld() : _isPause(false), _score(0) {
}
...
void HelloWorld::ccTouchesEnded(CCSet *touches, CCEvent *event) {
    ...
    if (distance > 100. && diffSelected < 800000.) { // success
        ...
        for (int i = 0; i < 5; i++) {
            if (pt.x - this->_selectedPoint.x < 0) {
                removingTableCloth->addObject(
                    CCSpriteFrame::createWithTexture(this->_tableClothTexture, CCRectMake(0, 200 * i, 300, 200)));

            } else {
                removingTableCloth->addObject(
                    CCSpriteFrame::createWithTexture(this->_tableClothTexture, CCRectMake(300, 200 * i, 300, 200)));
            }
        }
        ...
        CCCallFuncO *actionDone = CCCallFuncO::create(this, callfuncO_selector(HelloWorld::removeSprite),
                                                      (CCSprite *)selectedTableClothSprite);
    ...
    for (int i = 0; i < 4; i++) {
        if (afterSweep == 3) { // 깨짐
            shakeObjectOverTableClothSpriteFrames->addObject(
                CCSpriteFrame::createWithTexture(this->_objectOverTableCloth, CCRectMake(200, 200 * i, 200, 200)));
        } else { // 흔들흔들
            shakeObjectOverTableClothSpriteFrames->addObject(
                CCSpriteFrame::createWithTexture(this->_objectOverTableCloth, CCRectMake(0, 200 * i, 200, 200)));
        }
    }

    CCAnimation *animation = CCAnimation::createWithSpriteFrames(shakeObjectOverTableClothSpriteFrames, .2f);
    CCAnimate *animate = CCAnimate::create(animation);
    CCSprite *selectedObjectOverTableClothSprite =
        (CCSprite *)this->_objectOverTableClothSprites->objectAtIndex(this->_selectedIndex);
...
void HelloWorld::reset() { // 점수 초기화 및 테이블 리세팅
    ...
    float tableInformations[16][2] = {{100., 406.}, {330., 443.}, {560., 480.}, {790., 517.},
    ...
    CCSprite *highlightSprite = this->_selectedTableSprite =
        CCSprite::create("table_select.png", CCRectMake(0, 0, 200, 200));
    this->_selectedTableSprite->setVisible(false);
    ...
    for (unsigned int i = 0; i < tableCount; i++) {
        ...
        CCSpriteFrame *stableTableCloth =
            CCSpriteFrame::createWithTexture(this->_tableClothTexture, CCRectMake(0, 0, 300, 200));

        CCSprite *tableClothSprite = CCSprite::createWithSpriteFrame(stableTableCloth);

        // generate objects over table cloth
        CCSpriteFrame *stableObjectOverTableCloth =
            CCSpriteFrame::createWithTexture(this->_objectOverTableCloth, CCRectMake(0, 0, 200, 200));
        CCSprite *tableObject = CCSprite::createWithSpriteFrame(stableObjectOverTableCloth);

        // make table item instance
        TableItem *tableItem = new TableItem(tableSprite, tableClothSprite, tableObject, highlightSprite,
                                             ccp(tableInformations[i][0], tableInformations[i][1]));
...
#define TRANSFORM_SCALE 2.5
    ...
    CCMenuItemImage *btnPause =
        CCMenuItemImage::create("button_up.png", "button_down.png", this, menu_selector(HelloWorld::menuPauseCallback));
    btnPause->setPosition(ccp(CCDirector::sharedDirector()->getWinSize().width - 54,
                              CCDirector::sharedDirector()->getWinSize().height - 50));
